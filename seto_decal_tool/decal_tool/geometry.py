...
def sample_faces(bm, matrix_world):
    """Read every selected face of `bm` into world-space FaceSamples.
    ...
    for face in bm.faces:
        ...
        # World-space corner positions, used to measure how much room the face
        # offers so Random Position can keep the decal inside it.
        verts = [matrix_world @ v.co.copy() for v in face.verts]

        samples.append(FaceSample(center, normal, tangent, verts, face.index))
