...
Blender addon (built on [Sollumz](https://docs.sollumz.org)) for GTA V / FiveM MLO
work. Select faces, pick a decal from your library, press one button — you get
surface-aligned `decal.sps` decal planes, and your original mesh is never touched.
...
1. Download `seto_decal_tool.zip` (don't unzip).
2. Blender: **Edit > Preferences > Add-ons > Install from Disk** → pick the zip.
3. Enable it, then **restart Blender**.

It shares the **Seto Tools** N-panel tab with Seto Fake AO and Seto Fake Damage, but
does not depend on them — install whichever you want.
