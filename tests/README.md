...
| `updater.py` | the in-Blender updater: that the version maths pins garbage tags to "not newer", that the installable zip is picked over GitHub's source archives, that installing before checking and installing from any host but this repository's releases are both refused, and — the promise itself — that the update operators are the only code in the package that can reach the network and nothing in the updater runs on a timer or handler |
