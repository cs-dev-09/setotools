...
def apply_strip_bevel(mesh, edges, coords, settings):
    """Round off the strip's own seam, if Bevel Smooth Edge is ticked.
...
def _on_setting_changed(self, context):
    ...
    if obj is None or not isinstance(obj, bpy.types.Object):
        return
    if _rebuilding or not self.is_smooth_edge:
        return
    ...
    source_bevel.sync(self.source_object, leader=obj, tool=source_bevel.SMOOTH_EDGE)
...
def _object_annotations():
    ...
    annotations = {
        "is_smooth_edge": bpy.props.BoolProperty(
            name="Is Smooth Edge",
...
class SETO_PG_smooth_edge_object(bpy.types.PropertyGroup):
    __annotations__ = _object_annotations()


class SETO_OT_smooth_edge_rebuild(bpy.types.Operator):
    """Regenerate this Smooth Edge strip from its stored source edges and settings"""
    bl_idname = "seto.smooth_edge_rebuild"
    ...
    def poll(cls, context):
        ...
        return obj is not None and obj.type == 'MESH' and obj.seto_smooth_edge_data.is_smooth_edge

    def execute(self, context):
        obj = context.active_object
        message = rebuild(obj)
        obj.seto_smooth_edge_data.status = message or ""
        if message:
...
_classes = (
    SETO_PG_smooth_edge_object,
    SETO_OT_smooth_edge_rebuild,
)
...
def register():
    ...
    bpy.types.Object.seto_smooth_edge_data = bpy.props.PointerProperty(type=SETO_PG_smooth_edge_object)


def unregister():
    del bpy.types.Object.seto_smooth_edge_data
