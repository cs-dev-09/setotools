...
def _next_smooth_edge_name():
    """Explicit sequential naming (smooth_edge_001, _002, ...) instead of
    ...
    return f"smooth_edge_{max_n + 1:03d}"
...
class SETO_OT_create_smooth_edge(bpy.types.Operator):
    ...
    def execute(self, context):
        if not szi.is_sollumz_available():
            self.report({'ERROR'}, "Sollumz is not enabled/available. Seto Smooth Edge requires Sollumz.")
            return {'CANCELLED'}
        ...
        # flat, and a hard band at a quad boundary is exactly what this
        ...
        try:
            material, missing_params, texture_warning = szi.find_or_create_smooth_edge_material(
        ...
        try:
            ...
            self.report({'WARNING'}, f"Smooth Edge mesh created, but setting Origin to Geometry failed: {e}")
        ...
        with object_settings.suppress_rebuild():
            obj_data = new_obj.seto_smooth_edge_data
            obj_data.is_smooth_edge = True
        ...
        if shader_warning:
            self.report({'WARNING'}, f"Smooth Edge mesh created, but {szi.SMOOTH_EDGE_SHADER_FILENAME} assignment failed: {shader_warning}")
        elif missing_params:
            self.report({'WARNING'}, f"Shader parameter(s) not found on {szi.SMOOTH_EDGE_SHADER_FILENAME}: {', '.join(missing_params)}.")
...
_classes = (SETO_OT_create_smooth_edge,)
